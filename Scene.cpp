...
void GameScene::setPlayer(Triangle2 tr, vec2<float> pos) {
    player = std::make_unique<Player>(tr);
    player->moveTo(pos);
    //player = make_unique<Actor>(tr, myController);
}
void GameScene::addEnemy(Triangle2 tr, vec2<float> pos) {
    auto&& enemy =  EnemyBuffer.emplace_back(std::make_unique<Enemy>(tr));
    enemy->moveTo(pos);
...
void GameScene::enemy_act(Enemy* obj, float dt) {
    if (auto* ActObj = dynamic_cast<IActable*>(obj)) {
        if (!(obj->is_alive))
            return;
        obj->player_pos = player->sprite.position; // enemy always know where player located
        ActObj->act(dt);
...
void GameScene::bullet_enemy_collision(Bullet bullet, Enemy* enemy) {
    if (enemy->is_collide(bullet.sprite.position, 100) && bullet.is_alive && enemy->is_active ) {
...
void GameScene::addEnemies(int num) {
    for (int i = 0; i < num; ++i) {
        ...
        addEnemy(Triangle2(vec2<float>(15, 45), vec2<float>(0, 0), vec2<float>(30, 0), Color(200, 0, 0)), vec2<float>(randX, randY));
    }
